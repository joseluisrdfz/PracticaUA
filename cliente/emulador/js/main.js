$().ready(() => {
	...
	electro.on("connect", function () {
		...
		// Sonda
		$("#sondaConectada").click(() => electro.sondaConectada = !electro.sondaConectada);

		// Boleanos
		["luz", "resistenciaSuperior", "resistenciaInferior", "gratinador", "ventilador", "presencia", "sondaConectada", "puertaAbierta"].forEach(x => {
			$("#" + x).addClass("estado");
